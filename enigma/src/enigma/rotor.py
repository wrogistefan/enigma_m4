...
class Rotor:
    ...
    def __init__(
        self, wiring: str, notch: str | None, ring_setting: int = 0, position: str = "A"
    ) -> None:
        if len(wiring) != self.ALPHABET_SIZE:
            msg = "Rotor wiring must be 26 characters long."
            raise ValueError(msg)

        self.wiring = wiring.upper()
        self.notch: str | None = notch.upper() if notch is not None else None
    ...
    def step(self) -> bool:
        ...
        Return True if it is now at its notch.
        """
